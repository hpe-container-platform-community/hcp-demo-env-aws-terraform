...
ssh -q -o StrictHostKeyChecking=no -i "${LOCAL_SSH_PRV_KEY_PATH}" -T ubuntu@${RDP_PUB_IP} <<-EOF1
  ...
  kubectl --kubeconfig <(hpecp k8scluster --id $CLUSTER_ID admin-kube-config) \
    exec -c app -n $TENANT_NS \$POD -- chown bluedata:bluedata /opt/bluedata/hadoop-2.8.5/etc/hadoop/core-site.xml
    
  kubectl --kubeconfig <(hpecp k8scluster --id $CLUSTER_ID admin-kube-config) \
    exec -c app -n $TENANT_NS \$POD -- chmod 644 /opt/bluedata/hadoop-2.8.5/etc/hadoop/core-site.xml
