...
for MAPR_HOST in ${MAPR_HOSTS_PUB_IPS[@]}; do 
   ssh -o StrictHostKeyChecking=no -i "${LOCAL_SSH_PRV_KEY_PATH}" -T centos@${MAPR_HOST} 'echo MAPR_HOST: $(hostname)'
done
...
for MAPR_HOST in ${MAPR_HOSTS_PUB_IPS[@]}; do 
    ssh -o StrictHostKeyChecking=no -i "${LOCAL_SSH_PRV_KEY_PATH}" -T centos@${CTRL_PUB_IP} "cat /home/centos/.ssh/id_rsa.pub" | \
        ssh -o StrictHostKeyChecking=no -i "${LOCAL_SSH_PRV_KEY_PATH}" -T centos@${MAPR_HOST} "cat >> /home/centos/.ssh/authorized_keys"
done
...
for MAPR_HOST in ${MAPR_HOSTS_PRV_IPS[@]}; do 
    ssh -o StrictHostKeyChecking=no -i "${LOCAL_SSH_PRV_KEY_PATH}" -T centos@${CTRL_PUB_IP} << ENDSSH
        ...
        ssh -o ConnectTimeout=10 -o StrictHostKeyChecking=no -i ~/.ssh/id_rsa -T centos@${MAPR_HOST} "echo Connected to ${MAPR_HOST}!"
ENDSSH
done
...
ssh -o StrictHostKeyChecking=no -i "${LOCAL_SSH_PRV_KEY_PATH}" -T centos@${CTRL_PUB_IP} << ENDSSH
   ...
   cp -f mapr-ansible/host_templates/hosts_bash.sh hosts_bash.sh
   cp -f mapr-ansible/host_templates/hosts_cluster hosts_cluster.xml

   sed -i s/10.0.0.132/${MAPR_HOSTS_PRV_IPS[0]}/g ./hosts_cluster.xml
   sed -i s/ansible_user=ec2-user/ansible_user=centos/g ./hosts_cluster.xml
   sed -i '1,/\[mapr-opentsdb\]/!d' ./hosts_cluster.xml
