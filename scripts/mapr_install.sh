...
echo MAPR_CLUSTER1_HOSTS_PRV_IPS=${MAPR_CLUSTER1_HOSTS_PRV_IPS[@]}
echo MAPR_CLUSTER1_HOSTS_PUB_IPS=${MAPR_CLUSTER1_HOSTS_PUB_IPS[@]}
...
for MAPR_CLUSTER1_HOST in ${MAPR_CLUSTER1_HOSTS_PUB_IPS[@]}; do 
   ssh -o StrictHostKeyChecking=no -i "${LOCAL_SSH_PRV_KEY_PATH}" -T ubuntu@${MAPR_CLUSTER1_HOST} << ENDSSH
   sudo chmod -x /etc/update-motd.d/*
   touch .hushlogin
   echo MAPR_CLUSTER1_HOST: $(hostname)
ENDSSH
done
...
for MAPR_CLUSTER1_HOST in ${MAPR_CLUSTER1_HOSTS_PUB_IPS[@]}; do 
    ssh -o StrictHostKeyChecking=no -i "${LOCAL_SSH_PRV_KEY_PATH}" -T centos@${CTRL_PUB_IP} "cat /home/centos/.ssh/id_rsa.pub" | \
        ssh -o StrictHostKeyChecking=no -i "${LOCAL_SSH_PRV_KEY_PATH}" -T ubuntu@${MAPR_CLUSTER1_HOST} "cat >> /home/ubuntu/.ssh/authorized_keys"
done

# test passwordless SSH connection from Controller to Workers
for MAPR_CLUSTER1_HOST in ${MAPR_CLUSTER1_HOSTS_PRV_IPS[@]}; do 
    ssh -o StrictHostKeyChecking=no -i "${LOCAL_SSH_PRV_KEY_PATH}" -T centos@${CTRL_PUB_IP} << ENDSSH
        echo CONTROLLER: Connecting to MAPR_CLUSTER1_HOST ${MAPR_CLUSTER1_HOST}...
        ssh -o ConnectTimeout=10 -o StrictHostKeyChecking=no -i ~/.ssh/id_rsa -T ubuntu@${MAPR_CLUSTER1_HOST} "echo Connected to ${MAPR_CLUSTER1_HOST}!"
ENDSSH
done
...
ssh -o StrictHostKeyChecking=no -i "${LOCAL_SSH_PRV_KEY_PATH}" -T centos@${CTRL_PUB_IP} << ENDSSH
   ...
   sed -i s/10.0.0.114/${MAPR_CLUSTER1_HOSTS_PRV_IPS[0]}/g ./hosts_cluster.xml
   sed -i s/10.0.0.150/${MAPR_CLUSTER1_HOSTS_PRV_IPS[1]}/g ./hosts_cluster.xml
   sed -i s/10.0.0.162/${MAPR_CLUSTER1_HOSTS_PRV_IPS[2]}/g ./hosts_cluster.xml
