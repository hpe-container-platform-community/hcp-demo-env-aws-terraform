...
if [[ "$RDP_PUB_IP" && "$RDP_SERVER_OPERATING_SYSTEM" = "LINUX" && -f generated/controller.prv_key ]]; then
    ...
    cat generated/controller.prv_key | \
        ssh -o StrictHostKeyChecking=no -i "${LOCAL_SSH_PRV_KEY_PATH}" -T ubuntu@${RDP_PUB_IP} "cat > ~/.ssh/id_rsa" 

    ssh -o StrictHostKeyChecking=no -i "${LOCAL_SSH_PRV_KEY_PATH}" -T ubuntu@${RDP_PUB_IP} "chmod 600 ~/.ssh/id_rsa"

    cat generated/controller.prv_key | \
        ssh -o StrictHostKeyChecking=no -i "${LOCAL_SSH_PRV_KEY_PATH}" -T ubuntu@${RDP_PUB_IP} "cat > ~/Desktop/controller.prv_key" 
    ...
    ssh -o StrictHostKeyChecking=no -i "${LOCAL_SSH_PRV_KEY_PATH}" -T ubuntu@${RDP_PUB_IP} <<EOF1    
        cat > ~/.ssh/config <<EOF2
Host controller
...
EOF1
   ...
   ssh -o StrictHostKeyChecking=no -i "${LOCAL_SSH_PRV_KEY_PATH}" -T ubuntu@${RDP_PUB_IP} <<EOF
...
EOF
    ...
    if [[ "$AD_PUB_IP" ]]; then
        cat generated/controller.pub_key | \
            ssh -o StrictHostKeyChecking=no -i "${LOCAL_SSH_PRV_KEY_PATH}" -T centos@${AD_PUB_IP} "cat >> /home/centos/.ssh/authorized_keys" 
    fi
