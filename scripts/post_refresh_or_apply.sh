...
if [[ "$RDP_PUB_IP" && "$RDP_SERVER_OPERATING_SYSTEM" = "LINUX" && -f generated/controller.prv_key ]]; then
    ...
    cat generated/controller.prv_key | \
        ssh -q -o StrictHostKeyChecking=no -i "${LOCAL_SSH_PRV_KEY_PATH}" -T ubuntu@${RDP_PUB_IP} "cat > ~/Desktop/controller.prv_key" 
...
EOF
    ...
    ssh -q -o StrictHostKeyChecking=no -i "${LOCAL_SSH_PRV_KEY_PATH}" -T ubuntu@${RDP_PUB_IP} "echo ${WRKR_PRV_IPS[@]} > ~/Desktop/WORKER_HOSTS.txt"
